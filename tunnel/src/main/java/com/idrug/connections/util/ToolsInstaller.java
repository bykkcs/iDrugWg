...
package com.idrug.connections.util;

import android.content.Context;
import android.system.OsConstants;
import android.util.Log;

import com.idrug.connections.util.RootShell.RootShellException;
...
/**
 * Helper to install iDrugWG tools to the system partition.
 */
...
public final class ToolsInstaller {
    ...
    private static final String TAG = "iDrugWG/ToolsInstaller";
    ...
    public int install() throws RootShellException, IOException {
        if (!context.getPackageName().startsWith("org.iDrug."))
            throw new SecurityException("The tools may only be installed system-wide from the main iDrugWG app.");
    ...
    private int installMagisk() throws RootShellException, IOException {
        ...
        script.append("trap 'rm -rf /data/adb/modules/idrugwg' INT TERM EXIT; ");
        script.append(String.format("rm -rf /data/adb/modules/idrugwg/; mkdir -p /data/adb/modules/idrugwg%s; ", INSTALL_DIR));
        script.append("printf 'id=idrugwg\nname=iDrugWG Command Line Tools\nversion=1.0\nversionCode=1\nauthor=iDrug\ndescription=Command line tools for iDrugWG\nminMagisk=1500\n' > /data/adb/modules/idrugwg/module.prop; ");
        script.append("touch /data/adb/modules/idrugwg/auto_mount; ");
        for (final String name : EXECUTABLES) {
            final File destination = new File("/data/adb/modules/idrugwg" + INSTALL_DIR, name);
