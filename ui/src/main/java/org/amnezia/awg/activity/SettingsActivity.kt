...
private fun checkForOtaUpdate() {
    try {
        ...
        if (file.exists()) {
            ...
            if (!deleted) {
                Toast.makeText(context, getString(R.string.update_delete_old_file_error), Toast.LENGTH_SHORT).show()
            }
        }
        ...
        Toast.makeText(context, getString(R.string.update_download_started), Toast.LENGTH_SHORT).show()
        ...
        Toast.makeText(requireContext(), getString(R.string.update_launch_error, e.localizedMessage), Toast.LENGTH_LONG).show()
        e.printStackTrace()
    }
}
