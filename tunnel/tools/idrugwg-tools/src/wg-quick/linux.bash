...
parse_options() {
	...
	CONFIG_FILE="$1"
	[[ $CONFIG_FILE =~ ^[a-zA-Z0-9_=+.-]{1,15}$ ]] && CONFIG_FILE="/etc/iDrug/idrugwg/$CONFIG_FILE.conf"
...
add_if() {
	local ret
	if ! cmd ip link add "$INTERFACE" type idrugwg; then
		ret=$?
		[[ -e /sys/module/idrugwg ]] || ! command -v "${WG_QUICK_USERSPACE_IMPLEMENTATION:-idrugwg-go}" >/dev/null && exit $ret
		echo "[!] Missing WireGuard (iDrug VPN) kernel module. Falling back to slow userspace implementation." >&2
		cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-idrugwg-go}" "$INTERFACE"
	fi
}
...
cmd_usage() {
	...
	Usage: $PROGRAM [ up | down | save | strip ] [ CONFIG_FILE | INTERFACE ]
	  ...
	  configuration found at /etc/iDrug/INTERFACE.conf. It is to be readable
