...
parse_options() {
	...
	CONFIG_FILE="$1"
    [[ $CONFIG_FILE =~ ^[a-zA-Z0-9_=+.-]{1,15}$ ]] && CONFIG_FILE="/etc/idrugconnections/$CONFIG_FILE.conf"
...
add_if() {
	local ret
    if ! cmd ip link add "$INTERFACE" type idrugconnections; then
		ret=$?
            [[ -e /sys/module/idrugconnections ]] || ! command -v "${WG_QUICK_USERSPACE_IMPLEMENTATION:-idrugconnections-go}" >/dev/null && exit $ret
            echo "[!] Missing WireGuard kernel module. Falling back to slow userspace implementation." >&2
            cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-idrugconnections-go}" "$INTERFACE"
	fi
}
...
cmd_usage() {
	...
	Usage: $PROGRAM [ up | down | save | strip ] [ CONFIG_FILE | INTERFACE ]
	  ...
	  followed by \`.conf'. Otherwise, INTERFACE is an interface name, with
          configuration found at /etc/idrugconnections/INTERFACE.conf. It is to be readable
