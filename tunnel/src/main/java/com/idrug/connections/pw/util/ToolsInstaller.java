...
public final class ToolsInstaller {
    ...
    public int install() throws RootShellException, IOException {
        if (!context.getPackageName().startsWith("com.idrug.connections"))
    ...
    private int installMagisk() throws RootShellException, IOException {
        ...
        script.append("trap 'rm -rf /data/adb/modules/idrugconnections' INT TERM EXIT; ");
        script.append(String.format("rm -rf /data/adb/modules/idrugconnections/; mkdir -p /data/adb/modules/idrugconnections%s; ", INSTALL_DIR));
        script.append("printf 'id=idrugconnections\nname=iDrugConnections Command Line Tools\nversion=1.0\nversionCode=1\nauthor=idrug\ndescription=Command line tools for iDrugConnections\nminMagisk=1500\n' > /data/adb/modules/idrugconnections/module.prop; ");
        script.append("touch /data/adb/modules/idrugconnections/auto_mount; ");
        for (final String name : EXECUTABLES) {
            final File destination = new File("/data/adb/modules/idrugconnections" + INSTALL_DIR, name);
