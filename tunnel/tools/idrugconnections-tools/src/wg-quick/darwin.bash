...
CONFIG_SEARCH_PATHS=( /etc/idrugconnections /usr/local/etc/idrugconnections )
...
get_real_interface() {
    ...
    [[ -f "/var/run/idrugconnections/$INTERFACE.name" ]] || return 1
    interface="$(< "/var/run/idrugconnections/$INTERFACE.name")"
    [[ -n $interface && -S "/var/run/idrugconnections/$interface.sock" ]] || return 1
    diff=$(( $(stat -f %m "/var/run/idrugconnections/$interface.sock" 2>/dev/null || echo 200) - $(stat -f %m "/var/run/wireguard/$INTERFACE.name" 2>/dev/null || echo 100) ))
...
add_if() {
    export WG_TUN_NAME_FILE="/var/run/idrugconnections/$INTERFACE.name"
    mkdir -p "/var/run/idrugconnections/"
    cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-idrugconnections-go}" utun
	get_real_interface
}
