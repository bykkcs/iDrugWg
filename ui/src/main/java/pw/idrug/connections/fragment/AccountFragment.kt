...
class AccountFragment : Fragment() {
    ...
    private val qrScanLauncher = registerForActivityResult(
        ...
        if (result.resultCode == android.app.Activity.RESULT_OK && result.data != null) {
            ...
            if (!scanned.isNullOrEmpty()) {
                confirmQrLoginToken(scanned) { success, message ->
                    safeUi {
                        ...
                        Toast.makeText(
                            ctx,
                            if (success)
                                ctx.safeGetString(R.string.toast_qr_login_confirmed)
                            else
                                ctx.safeGetString(R.string.toast_confirmation_error, message ?: ""),
                            Toast.LENGTH_SHORT
                        ).show()
                ...
                Toast.makeText(
                    requireContext(),
                    requireContext().safeGetString(R.string.toast_qr_code_not_recognized),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    private fun setupListeners(view: View) {
        ...
        view.findViewById<Button>(R.id.btn_renew).setOnClickListener {
            ...
            if (token == null) {
                Toast.makeText(
                    requireContext(),
                    requireContext().safeGetString(R.string.toast_login_via_telegram_first),
                    Toast.LENGTH_SHORT
                ).show()
                setLoading(false)
            ...
            renewSubscription { success, resp ->
                safeUi {
                    ...
                    Toast.makeText(
                        ctx,
                        if (success)
                            ctx.safeGetString(R.string.toast_subscription_renewed)
                        else
                            ctx.safeGetString(R.string.toast_error_with_message, resp ?: ""),
                        Toast.LENGTH_SHORT
                    ).show()
                    if (success) loadProfileAndSetupUI(requireView())
                }
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                safeUi {
                    ...
                    Toast.makeText(
                        requireContext(),
                        requireContext().safeGetString(R.string.toast_network_error, e.message ?: ""),
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
                ...
                safeUi {
                    ...
                    if (response.code == 401) {
                        ...
                        try {
                            ...
                            Toast.makeText(
                                requireContext(),
                                requireContext().safeGetString(
                                    R.string.toast_profile_processing_error,
                                    e.message ?: ""
                                ),
                                Toast.LENGTH_SHORT
                            ).show()
                        ...
                        Toast.makeText(
                            requireContext(),
                            requireContext().safeGetString(
                                R.string.toast_failed_retrieve_profile,
                                response.code
                            ),
                            Toast.LENGTH_SHORT
                        ).show()
    ...
    private fun afterLogout(view: View) {
        ...
        MainScope().launch {
            ...
            safeUi {
                ...
                Toast.makeText(
                    requireContext(),
                    requireContext().safeGetString(R.string.toast_logged_out),
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
    ...
    private fun handleDownloadConfig(view: View) {
        if (selectedServerId == null) {
            Toast.makeText(
                requireContext(),
                requireContext().safeGetString(R.string.toast_select_server),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        if (!subscriptions.any { it.location == serverId && it.active }) {
            Toast.makeText(
                requireContext(),
                requireContext().safeGetString(R.string.toast_subscription_inactive),
                Toast.LENGTH_SHORT
            ).show()
            return
        }
        ...
        if (token == null) {
            Toast.makeText(
                requireContext(),
                requireContext().safeGetString(R.string.login_via_telegram),
                Toast.LENGTH_SHORT
            ).show()
            setLoading(false)
            return
        ...
        MainScope().launch {
            ...
            if (tunnel != null) {
                safeUi {
                    Toast.makeText(
                        requireContext(),
                        requireContext().safeGetString(R.string.toast_config_already_added),
                        Toast.LENGTH_SHORT
                    ).show()
                    setLoading(false)
                }
                    ...
                    if (success) {
                        ...
                        MainScope().launch {
                            try {
                                ...
                                Toast.makeText(
                                    requireContext(),
                                    requireContext().safeGetString(R.string.toast_tunnel_added),
                                    Toast.LENGTH_SHORT
                                ).show()
                                ...
                                Toast.makeText(
                                    requireContext(),
                                    requireContext().safeGetString(
                                        R.string.toast_tunnel_creation_error,
                                        e.message ?: ""
                                    ),
                                    Toast.LENGTH_LONG
                                ).show()
                        ...
                        Toast.makeText(
                            requireContext(),
                            requireContext().safeGetString(R.string.toast_error_with_message, configOrError ?: ""),
                            Toast.LENGTH_LONG
                        ).show()
                    }
                }
    ...
    private fun showQrCode(token: String, imageView: ImageView) {
        try {
            ...
            Toast.makeText(
                requireContext(),
                requireContext().safeGetString(R.string.toast_qr_code_generation_error),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
                ...
                pollQrLoginStatus(token) { confirmed, jwt, username, photoUrl ->
                    if (confirmed && jwt != null && username != null) {
                        ...
                        safeUi {
                            Toast.makeText(
                                requireContext(),
                                requireContext().safeGetString(R.string.toast_qr_login_confirmed),
                                Toast.LENGTH_SHORT
                            ).show()
                            loadProfileAndSetupUI(requireView())
                        }
    ...
    private fun renewSubscription(callback: (Boolean, String?) -> Unit) {
        ...
        try {
            ...
            Toast.makeText(
                requireContext(),
                requireContext().safeGetString(R.string.toast_unable_open_telegram),
                Toast.LENGTH_SHORT
            ).show()
        }
    }
        ...
        linkAccountWithCode(code) { success, token, username, message ->
            safeUi {
                if (success && token != null && username != null) {
                    ...
                    Toast.makeText(
                        requireContext(),
                        requireContext().safeGetString(R.string.toast_login_successful),
                        Toast.LENGTH_SHORT
                    ).show()
                    ...
                    Toast.makeText(
                        requireContext(),
                        message ?: requireContext().safeGetString(R.string.login_via_telegram_or_qr),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
    ...
    private fun handleDeepLink(intent: Intent?) {
        ...
        if (data != null && data.scheme == "idrug" && data.host == "auth") {
            ...
            if (!jwt.isNullOrEmpty() && !username.isNullOrEmpty()) {
                ...
                Toast.makeText(
                    requireContext(),
                    requireContext().safeGetString(R.string.toast_telegram_login_successful),
                    Toast.LENGTH_SHORT
                ).show()
                showCorrectScreen(requireView())
            }
