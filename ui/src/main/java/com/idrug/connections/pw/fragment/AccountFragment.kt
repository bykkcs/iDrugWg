...
class AccountFragment : Fragment() {
    ...
    private fun setupListeners(view: View) {
        ...
        view.findViewById<Button>(R.id.btn_show_code_login).setOnClickListener {
            setLoading(true)
            generateLinkCode { code ->
                safeUi {
                    setLoading(false)
                    if (code == null) {
                        Toast.makeText(requireContext(), "Ошибка получения кода", Toast.LENGTH_SHORT).show()
                    } else {
                        view.findViewById<TextView>(R.id.link_code_text).apply {
                            text = code
        ...
        view.findViewById<Button>(R.id.btn_login_code).setOnClickListener {
            val code = view.findViewById<EditText>(R.id.edit_link_code).text.toString()
            if (code.length != 6) {
                Toast.makeText(requireContext(), "Введите корректный код", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            setLoading(true)
            consumeLinkCode(code) { success, jwt, username, photoUrl ->
                safeUi {
                    setLoading(false)
                    if (success && jwt != null && username != null) {
                        prefs.edit()
                            .putString("token", jwt)
                        ...
                        Toast.makeText(requireContext(), "Код не принят", Toast.LENGTH_SHORT).show()
                    }
                }
            }
        }
        ...
        // code login does not require scanner
    }
    ...
    private fun showLoginScreen(view: View) {
        ...
        view.findViewById<Button>(R.id.btn_show_code_login).visibility = View.VISIBLE
        view.findViewById<TextView>(R.id.link_code_text).visibility = View.GONE
        view.findViewById<EditText>(R.id.edit_link_code).visibility = View.VISIBLE
        view.findViewById<Button>(R.id.btn_login_code).visibility = View.VISIBLE
        ...
        view.findViewById<TextView>(R.id.link_code_text).visibility = View.GONE
        view.findViewById<TextView>(R.id.text_current_user).text = "Вход через Telegram или код"
    ...
    private fun showAccountScreen(view: View, username: String, photoUrl: String?, status: String, expDateStr: String?) {
        ...
        view.findViewById<Button>(R.id.btn_show_code_login).visibility = View.VISIBLE
        view.findViewById<Button>(R.id.btn_login_code).visibility = View.GONE
        view.findViewById<EditText>(R.id.edit_link_code).visibility = View.GONE
        view.findViewById<TextView>(R.id.link_code_text).visibility = View.GONE
...
private fun afterLogout(view: View) {
    ...
    // Генерация и потребление кодов быстрого входа
    private fun generateLinkCode(callback: (String?) -> Unit) {
        val jwt = prefs.getString("token", null)
        if (jwt == null) return callback(null)
        val request = Request.Builder()
        ...
        OkHttpClient().newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    ...
                    callback(json.optString("link_code", null))
                } else callback(null)
            }
        })
    }

    private fun consumeLinkCode(code: String, callback: (Boolean, String?, String?, String?) -> Unit) {
        ...
        val request = Request.Builder()
            ...
            .build()
        OkHttpClient().newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) { callback(false, null, null, null) }
            override fun onResponse(call: Call, response: Response) {
                if (response.isSuccessful) {
                    val json = JSONObject(response.body?.string() ?: "{}")
                    val jwt = json.optString("jwt", null)
                    val username = json.optString("username", null)
                    callback(jwt != null && username != null, jwt, username, null)
                } else callback(false, null, null, null)
            }
        })
    }
