...
CONFIG_SEARCH_PATHS=( /etc/iDrug/idrugwg /usr/local/etc/iDrug/idrugwg )
...
get_real_interface() {
	...
	[[ -f "/var/run/idrugwg/$INTERFACE.name" ]] || return 1
	interface="$(< "/var/run/idrugwg/$INTERFACE.name")"
	[[ -n $interface && -S "/var/run/idrugwg/$interface.sock" ]] || return 1
	diff=$(( $(stat -f %m "/var/run/idrugwg/$interface.sock" 2>/dev/null || echo 200) - $(stat -f %m "/var/run/wireguard/$INTERFACE.name" 2>/dev/null || echo 100) ))
...
add_if() {
	export WG_TUN_NAME_FILE="/var/run/idrugwg/$INTERFACE.name"
	mkdir -p "/var/run/idrugwg/"
	cmd "${WG_QUICK_USERSPACE_IMPLEMENTATION:-idrugwg-go}" utun
	get_real_interface
}
