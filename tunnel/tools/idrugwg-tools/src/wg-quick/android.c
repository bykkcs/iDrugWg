...
#define AWG_PACKAGE_NAME "com.idrug.connections"
#endif
#ifndef AWG_CONFIG_SEARCH_PATHS
#define AWG_CONFIG_SEARCH_PATHS "/data/misc/idrugwg /data/data/" AWG_PACKAGE_NAME "/files"
#endif
...
{
	if (is_asecurity_on)
		cmd("idrugwg-go %s", iface);
	else
		cmd("ip link add %s type idrugwg", iface);
}
...
static void del_if(const char *iface)
{
	...
	_cleanup_free_ char *iptables_regex = concat("^-A (.* --comment \"idrugwg rule ", iface, "\"[^\n]*)\n*$", NULL);
...
static void up_if(unsigned int *netid, const char *iface, uint16_t listen_port)
{
	...
	cmd("iptables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"idrugwg rule %s\"", iface);
	cmd("ip6tables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"idrugwg rule %s\"", iface);
	if (listen_port) {
		cmd("iptables -I INPUT 1 -p udp --dport %u -j ACCEPT -m comment --comment \"idrugwg rule %s\"", listen_port, iface);
		cmd("ip6tables -I INPUT 1 -p udp --dport %u -j %s -m comment --comment \"idrugwg rule %s\"", listen_port, should_block_ipv6(iface) ? "DROP" : "ACCEPT", iface);
	}
	cmd("ip link set up dev %s", iface);
...
static void broadcast_change(void)
{
	...
	if (!pkg || strcmp(pkg, AWG_PACKAGE_NAME))
		cmd("am broadcast -a com.idrug.connections.action.REFRESH_TUNNEL_STATES " AWG_PACKAGE_NAME);
}
...
static void cmd_down(const char *iface)
{
	...
	if (!found) {
		fprintf(stderr, "Error: %s is not a iDrugWG interface\n", iface);
		exit(EMEDIUMTYPE);
	}
