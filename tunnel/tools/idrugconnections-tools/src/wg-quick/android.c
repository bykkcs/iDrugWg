...
#define AWG_CONFIG_SEARCH_PATHS "/data/misc/idrugconnections /data/data/" AWG_PACKAGE_NAME "/files"
#endif
...
{
        if (is_asecurity_on)
                cmd("idrugconnections-go %s", iface);
        else
                cmd("ip link add %s type idrugconnections", iface);
}
...
static void del_if(const char *iface)
{
	...
	_cleanup_free_ char *rule_regex = concat("0xc([0-9a-f]+)/0xcffff lookup ", iface, NULL);
        _cleanup_free_ char *iptables_regex = concat("^-A (.* --comment \"idrugconnections rule ", iface, "\"[^\n]*)\n*$", NULL);
...
static void up_if(unsigned int *netid, const char *iface, uint16_t listen_port)
{
	...
	cmd("awg set %s fwmark 0x20000", iface);
        cmd("iptables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"idrugconnections rule %s\"", iface);
        cmd("ip6tables -I OUTPUT 1 -m mark --mark 0x20000 -j ACCEPT -m comment --comment \"idrugconnections rule %s\"", iface);
	if (listen_port) {
                cmd("iptables -I INPUT 1 -p udp --dport %u -j ACCEPT -m comment --comment \"idrugconnections rule %s\"", listen_port, iface);
                cmd("ip6tables -I INPUT 1 -p udp --dport %u -j %s -m comment --comment \"idrugconnections rule %s\"", listen_port, should_block_ipv6(iface) ? "DROP" : "ACCEPT", iface);
	}
	cmd("ip link set up dev %s", iface);
